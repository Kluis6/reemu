...
# Rust.
#
# `reemu-core-host` (o processo filho que carrega o core) é um `[[bin]]` de
# `core-host-desktop`, crate de que o `reemu-desktop` NÃO depende. Quem o
# recompila é o `beforeDevCommand` do `tauri.conf.json` — então `cargo tauri
# dev` direto também serve; este script só junta o `--config` e os atalhos
# abaixo (RUST_LOG, validation layer).
#
# Uso:
